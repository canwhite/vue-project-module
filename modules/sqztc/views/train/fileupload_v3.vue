...
<script>
	...
	export default {
	  ...
	  mounted(){
	  	let that = this;
	  	if(sessionStorage.editHistory2!=undefined){
	  		that.params = JSON.parse(sessionStorage.editHistory2)
	  	}
	  },
	  beforeRouteLeave(to, from, next){
	  	var cacheObj = this.params;
	  	sessionStorage.setItem('editHistory2',JSON.stringify(cacheObj));
	  	next();
	  },
	  beforeDestroy(){
	  	try{
	  ...
	  data() {
	  	return {
			...
			placeholder:[
				"针对省区日常经营、公司制度文件或他人经验积累等方面进行咨询，问您想问",
				"就企业发展、省区经营、产品营销、技术创新、降低成本、提高效益等方面都可以提出合理化建议",
	  		...
	  		$.ajax({
	            ...
	            success: function (res) {  
					...
					if(res.success){
		            	...
		            	that.$destroy();
		            	sessionStorage.removeItem('editHistory2');
		            }else{
